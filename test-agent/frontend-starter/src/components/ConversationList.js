...
const ConversationList = ({ conversations = [], loading, error, onRetry }) => {
  ...
  const filteredConversations = useMemo(() => {
    return conversations.filter((conv) => {
      ...
      return matchesSearch && matchesStatus && matchesAlert && matchesAgent;
    });
  ...
  const uniqueAgents = useMemo(() => {
    ...
    conversations.forEach(conv => {
      if (conv.agentId && conv.agentName) {
        agents.set(conv.agentId, conv.agentName);
      }
    });
  ...
  if (error) {
    return (
      <Box p={4} textAlign="center">
        <Text color="red.500" mb={4}>Error loading conversations: {error}</Text>
        {onRetry && (
        ...
        {filteredConversations.length === 0 ? (
          ...
          filteredConversations.map((conversation) => (
            <Box
              ...
              bg={(conversation.status === 'escalated' || conversation.alertLevel === 'high') ? 'red.50' : 'transparent'}
